...
import com.example.springai.stateful.domain.PlanStep;
 ...
 *
 * <p>@FW-CMP [TOOL_LOOP] 外层 Plan 循环 Spring AI 不管，只能自建
 ...
 *     // ToolCallingAdvisor 只接管内层小循环（一次 .call()）；
 *     // 外层"哪步走完走哪步、失败了重规划、状态落库"没有框架概念，
 *     // 本类就是那个外层循环
 *   差异：框架不是"接管一切"，而是"在特定层接管"；
 *   本章最重要的分界线就在这里：内层框架管，外层自己写。
 *   完整版见 docs/comparisons/ch03_spring_ai_stateful.md#3-tool-loop-outer
 */
